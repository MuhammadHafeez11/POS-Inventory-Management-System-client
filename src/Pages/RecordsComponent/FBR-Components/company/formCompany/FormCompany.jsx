...
import { postPCTCodeCompany } from "../../../../../actions/pctCodeAction";
...
const FormCompany = () => {
  ...
  useEffect(() => {
    isCalledd = "false";
  }, [isCalledd, companyName, companyAddress, pctCodeCompany]);
  ...
  const AddCompany = async () => {
    ...
    pctCodeCompany !== "No Record Found" && pctCodeCompany?.map((company) => {
    ...
    if (productMatch === "true") {
      ...
      const response = await postPCTCodeCompany(companyName);
  ...
  useEffect(() => {
    if (pctCodeCompany?.length > 0) {
      console.log(pctCodeCompany);
      setData(pctCodeCompany);
      setCompanyLoading(false);
    }else 
      if(!pctCodeCompanyLoading) {
        setCompanyLoading(false)
      }
  ...
  const handleSearch = async (companyName) => {
    ...
    const dataa = await SearchCompanyData(pctCodeCompany, companyName);
    setData(dataa);
  };
    ...
    <>
      ...
      <div className={`Company ${colorTheme}`}>
        ...
        <div className="contentt-box">
          ...
          {!companyLoading && pctCodeCompany !== "No Record Found" ? (
            <TableComponentId
              data={data}
              columns={columns}
            />
