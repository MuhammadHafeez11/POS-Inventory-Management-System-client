...
const TableTransfer = () => {
  ...
  const [defaultSerialNumber, setDefaultSerialNumber] = useState()
  ...
  const getSalesRecord = async () => {
    ...
    if (saleRecord) {
      ...
      if (
        ...
        serialNumberList =pr?.map(storage =>({
          value: storage.serialNumber,
          label: storage.serialNumber 
        }))
        setData(pr);
  ...
  const loadSerialNumberOptions = (search, callBack) => {
    setTimeout(()=>{
      const filterOptions = serialNumberList?.filter(option=> option?.value.toLowerCase().includes(search.toLowerCase()))
      callBack(filterOptions)
    }, 3000)
  }
  ...
  const handleSerialNumberSelectChange = ( value ) => {
    if(value)
      {
        setSerialNumber(value?.value);
    setDefaultSerialNumber(value)
      }else {
  ...
  const columns = [
    ...
    { field: "serialNumber", label: t("serialNumber") },
      ...
      <div className="secondContainer">
        ...
        <AsyncSelect 
          ...
          {/* <AsyncSelect 
            ...
            ref={selectInputRef3}
            loadOptions={serialNumberList?.length > 0 && loadSerialNumberOptions}
            defaultOptions={serialNumberList} onChange={handleSerialNumberSelectChange}
            defaultValue={defaultSerialNumber}
                   /> */}
               <DateRangePicker
                 showOneCalendar
                size="lg"
