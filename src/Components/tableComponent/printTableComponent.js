...
const PrintTableComponent = ({
  ...
  return (
    <div style={{ width: "100%", fontSize: "8px", marginTop: "10px"}}>
      {data && data?.length > 0 ? (
        <table
          style={{
            marginTop: "0px",
          ...
          <thead>
            <tr>
              {firstHalfColumns?.map((column) => (
                <th
                  key={column.field}
                  style={{ padding: "0px", fontSize: "10px", textAlign: "center" }}
                >
                  {column.label}
                </th>
              ))}
          ...
          {action4 === "salePage" ? (
            <tfoot>
              <tr>
                ...
                >
                  {/* Your footer cell content */}
                  {list?.length > 0 ?  GrandTotalExludeTex.toLocaleString() : null}
                </td>
