...
export const getCompany = () => async (dispatch) => {
  try {
    ...
    const { data } = await axiosInstance.get("/api/company/get");
    dispatch({
...
export const getCompanyDetails = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axiosInstance.get(`/api/company/get/${id}`);
    dispatch({
...
export const deleteCompany = async (id) => {
  try {
    const config = { headers: { "Content-Type": "application/json" } };
    const { data } = await axiosInstance.delete(`/api/company/delete/${id}`);
    return data;
  } catch (error) {
    //    console.log(error.response)
